		...
		function wm_theme_customizer( $wp_customize ) {
			...
			// Processing
				...
				// Change background image section title & priority
					...
					$wp_customize->get_section( 'background_image' )->title    = esc_html_x( 'Background', 'Customizer section title.', 'glamour' );
					$wp_customize->get_section( 'background_image' )->priority = 30;

				// Change header image section title & priority

					$wp_customize->get_section( 'header_image' )->title    = esc_html_x( 'Header', 'Customizer section title.', 'glamour' );
 ...
 */
	 ...
	 *
	 * Caching $replacement into 'glamour-customizer-values' transient.
	 * Caching $output into 'glamour-custom-css' transient.
	 *
	 * @since    1.0
	 * @version  2.0
	 *
	...
	if ( ! function_exists( 'wm_custom_styles' ) ) {
		function wm_custom_styles( $set_cache = false, $return = true ) {
			...
			// Helper variables
				...
				$replacements  = array_filter( array_filter( (array) get_transient( 'glamour-customizer-values' ) ) ); // There have to be values (defaults) set!
			...
			// Processing
				 ...
				 */
					...
					if (
						...
						if (
							...
							set_transient( 'glamour-customizer-values', $replacements );
						}
				...
				// Prepare output and cache
					...
					$output_cached = (string) get_transient( 'glamour-custom-css' );

					// Debugging set (via "debug" URL parameter)

						if ( isset( $_GET['debug'] ) ) {
							$output_cached = (string) get_transient( 'glamour-custom-css-debug' );
						}
					...
					if (
						...
						if ( $set_cache ) {
							set_transient( 'glamour-custom-css-debug', apply_filters( 'wmhook_wm_custom_styles_output_cache_debug', $output ) );
							set_transient( 'glamour-custom-css', apply_filters( 'wmhook_wm_custom_styles_output_cache', $output ) );
						}
		...
		if ( ! function_exists( 'wm_custom_styles_transient_flusher' ) ) {
			function wm_custom_styles_transient_flusher() {
				...
				// Processing
					...
					delete_transient( 'glamour-customizer-values' );
					delete_transient( 'glamour-custom-css-debug' );
					delete_transient( 'glamour-custom-css' );
