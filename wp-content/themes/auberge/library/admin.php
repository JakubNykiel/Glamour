 ...
 */
	 ...
	 *
	 * Displays the message stored in `glamour_admin_notice` transient cache
	...
	if ( ! function_exists( 'wm_admin_notices' ) ) {
		function wm_admin_notices() {
			...
			// Helper variables
				...
				$message    = get_transient( 'glamour_admin_notice' );
			...
			// Processing
				...
				if ( $message[0] && current_user_can( $message[2] ) ) {
					...
					delete_transient( 'glamour_admin_notice' );
				}
				...
				// Delete the transient cache after specific number of displays
					...
					if ( 1 < intval( $message[3] ) ) {
						...
						set_transient( 'glamour_admin_notice', $message, ( 60 * 60 * 48 ) );
					}
