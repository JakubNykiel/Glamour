			...
			// Helper variables
				...
				// Google Fonts
					...
					$output['google-fonts'] = array(
							...
							// No Google Font
								...
								' ' => esc_html__( ' - do not use Google Font', 'glamour' ),

							// Default theme font

								'optgroup' . $i  => esc_html_x( 'Theme default', 'Google Font default setup options group title.', 'glamour' ),
									'Ubuntu:400,300' => 'Ubuntu',
								'/optgroup' . $i => '',

							// Insipration from http://femmebot.github.io/google-type/

								'optgroup' . ++$i  => sprintf( esc_html_x( 'Recommendation #%d', 'Google Font setup recommendation (numbered) group title.', 'glamour' ), $i ),
							...
							// Google Fonts selection
								...
								'optgroup' . ++$i  => esc_html_x( 'Fonts selection', 'Title for selection of fonts picked from Google Fontss', 'glamour' ),
				...
				// Widget areas
					...
					$output['widget-areas'] = array(
							'sidebar' => array(
								'name'        => esc_html__( 'Sidebar', 'glamour' ),
								'description' => esc_html__( 'Page sidebar.', 'glamour' ),
							),
							'front-page'  => array(
								'name'        => esc_html__( 'Front Page Widgets', 'glamour' ),
								'description' => esc_html__( 'This widgets area is displayed below the Banner area on the front page (homepage).', 'glamour' ),
							),
							'footer'  => array(
								'name'        => esc_html__( 'Footer Widgets', 'glamour' ),
								'description' => esc_html__( 'Masonry layout is used to display footer widgets.', 'glamour' ),
							),
						);
			...
			// Processing
				...
				// Localization
					...
					// wp-content/languages/theme-name/it_IT.mo
						...
						load_theme_textdomain( 'glamour', trailingslashit( WP_LANG_DIR ) . 'themes/glamour' );

					// wp-content/themes/child-theme-name/languages/it_IT.mo

						load_theme_textdomain( 'glamour', get_stylesheet_directory() . '/languages' );

					// wp-content/themes/theme-name/languages/it_IT.mo

						load_theme_textdomain( 'glamour', get_template_directory() . '/languages' );

				// Custom menus

					register_nav_menus( apply_filters( 'wmhook_wm_setup_menus', array(
							'primary' => esc_html__( 'Primary Menu', 'glamour' ),
							'social'  => esc_html__( 'Social Links Menu', 'glamour' ),
						) ) );
				...
				// Custom header
					...
					add_theme_support( 'custom-header', apply_filters( 'wmhook_wm_setup_custom_header_args', array(
							...
							'header-text'   => false,
							'width'         => ( isset( $image_sizes['glamour_banner'] ) ) ? ( $image_sizes['glamour_banner'][0] ) : ( 1920 ),
							'height'        => ( isset( $image_sizes['glamour_banner'] ) ) ? ( $image_sizes['glamour_banner'][1] ) : ( 1080 ),
							'flex-height'   => false,
							'flex-width'    => false,
						) ) );
		...
		function wm_theme_v2_upgrade_notice( $current_theme_version, $new_theme_version ) {
			...
			// Processing
				...
				if (
					...
					set_transient(
							'glamour_admin_notice',
							array(
								esc_html__( 'Thank you for upgrading your theme.', 'glamour' )
								. ' ' . esc_html__( 'We have noticed you are using a child theme.', 'glamour' )
								. '<br><strong>'
								. esc_html__( 'Please note that there has been a lot of changes in this new theme version and you should check the code in your child theme for compatibility!', 'glamour' )
								. '</strong>'
								. '<br><a href="https://github.com/webmandesign/glamour/blob/master/changelog.md" target="_blank">'
								. esc_html__( 'Read theme changelog &raquo;', 'glamour' )
								. '</a>',
								'notice-error',
								'switch_themes',
								3
				...
				// Processing
					...
					$image_sizes = array(
							'thumbnail' => array(
									...
									280,
									true,
									esc_html__( 'In posts list.', 'glamour' )
								),
							'medium' => array(
							...
							'large' => array(
									...
									9999,
									false,
									esc_html__( 'In single post page.', 'glamour' )
								),
							'glamour_banner' => array(
									1640,
									686, //@link http://en.wikipedia.org/wiki/Anamorphic_format
									true,
									esc_html__( 'In front page banner.', 'glamour' )
								),
							'glamour_banner_small' => array(
									absint( $content_width ),
									absint( $content_width / 2.39 ), //@link http://en.wikipedia.org/wiki/Anamorphic_format
									true,
									esc_html__( 'In food menu list.', 'glamour' )
								),
						);
			...
			function wm_image_size_notice_html() {
				...
				// Helper variables
					...
					$default_image_size_names = array(
							'thumbnail' => esc_html_x( 'Thumbnail size', 'WordPress predefined image size name.', 'glamour' ),
							'medium'    => esc_html_x( 'Medium size', 'WordPress predefined image size name.', 'glamour' ),
							'large'     => esc_html_x( 'Large size', 'WordPress predefined image size name.', 'glamour' ),
						);
				...
				// Output
					...
					// Section HTML
						...
						echo '<div class="recommended-image-sizes">';
							...
							echo '<h3>' . esc_html__( 'Recommended image sizes', 'glamour' ) . '</h3>'
								. '<p>' . esc_html__( 'For the theme to work correctly, please, set these recommended image sizes:', 'glamour' ) . '</p>';

							echo '<table>';

								echo '<thead>'
									. '<tr>'
									. '<th>' . esc_html__( 'Size name', 'glamour' ) . '</th>'
									. '<th>' . esc_html__( 'Size parameters', 'glamour' ) . '</th>'
									. '<th>' . esc_html__( 'Theme usage', 'glamour' ) . '</th>'
									. '</tr>'
									. '</thead>';
								...
								echo '<tbody>';
									...
									foreach ( $image_sizes as $size => $setup ) {
										...
										$crop = ( $setup[2] ) ? ( esc_html__( 'cropped', 'glamour' ) ) : ( esc_html__( 'scaled', 'glamour' ) );
										...
										if ( isset( $default_image_size_names[ $size ] ) ) {
											...
											echo '<tr title="' . esc_attr__( 'Additional image size added by the theme. Can not be changed on this page.', 'glamour' ) . '"><th><code>' . esc_html( $size ) . '</code>:</th>';

										}

										echo '<td>' . sprintf(
												esc_html_x( '%1$d &times; %2$d, %3$s', '1: image width, 2: image height, 3: cropped or scaled?', 'glamour' ),
												absint( $setup[0] ),
												absint( $setup[1] ),
												$crop
											) . '</td>'
			...
			function wm_custom_mce_format_addon( $style_formats = array() ) {
				...
				// Processing
					...
					$style_formats['100text_styles']['items']['100text_styles090'] = array(
							'title'    => esc_html__( 'Button link', 'glamour' ),
							'selector' => 'a',
							'classes'  => 'button',
							'icon'     => 'link',
						);
		...
		function wm_enqueue_assets() {
			...
			// Processing
				...
				// Scripts - inline
					...
					$scripts_inline = apply_filters( 'wmhook_wm_enqueue_assets_scripts_inline', array( 'text_menu_group_nav' => esc_html_x( '&uarr; Menu sections', 'Back to food menu sections selectors button title.', 'glamour' ) ) );
		...
		function wm_post_nav_background() {
			...
			// Processing
				...
				if ( $previous &&  has_post_thumbnail( $previous->ID ) ) {
					$prevthumb = wp_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), 'glamour_banner_small' );
					$output .= '.post-navigation .nav-previous { background-image: url(\'' . esc_url( $prevthumb[0] ) . '\'); }';
				}

				if ( $next && has_post_thumbnail( $next->ID ) ) {
					$nextthumb = wp_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), 'glamour_banner_small' );
				...
				// Output
					...
					if (
						...
						echo '<div class="breadcrumbs-container">'
						     ...
						     . '<h2 class="screen-reader-text" id="breadcrumbs-label-' . absint( $id_index ) . '">' . esc_attr__( 'Breadcrumbs navigation', 'glamour' ) . '</h2>'
						     . bcn_display( true )
						     . '</nav>'
						     . '</div>';
				...
				// Output
					...
					if (
							...
							the_post_navigation( array(
									'prev_text' => '<span class="label">' . esc_html__( 'Previous post', 'glamour' ) . '</span> <span class="title">%title</span>',
									'next_text' => '<span class="label">' . esc_html__( 'Next post', 'glamour' ) . '</span> <span class="title">%title</span>',
								) );
		...
		if ( ! function_exists( 'wm_sticky_label' ) ) {
			function wm_sticky_label() {
				...
				// Output
					...
					if ( is_sticky() ) {
						echo '<div class="label-sticky" title="' . esc_attr__( 'This is sticky post', 'glamour' ) . '"><i class="genericon genericon-pinned"></i></div>';
					}
			...
			function wm_excerpt( $excerpt ) {
				...
				// Requirements check
					...
					if ( post_password_required() ) {
						if ( ! is_single() ) {
							return esc_html__( 'This content is password protected.', 'glamour' ) . ' <a href="' . esc_url( get_permalink() ) . '">' . esc_html__( 'Enter the password to view it.', 'glamour' ) . '</a>';
						}
						return;
					}
		...
		if ( ! function_exists( 'wm_pagination' ) ) {
			function wm_pagination() {
				...
				// Helper variables
					...
					$pagination = array(
							'prev_text' => esc_html_x( '&laquo;', 'Pagination text (visible): previous.', 'glamour' ) . '<span class="screen-reader-text"> '
							               . esc_html_x( 'Previous page', 'Pagination text (hidden): previous.', 'glamour' ) . '</span>',
							'next_text' => '<span class="screen-reader-text">' . esc_html_x( 'Next page', 'Pagination text (hidden): next.', 'glamour' )
							               . ' </span>' . esc_html_x( '&raquo;', 'Pagination text (visible): next.', 'glamour' ),
						);
				...
				// Processing
					...
					if ( $output = paginate_links( $pagination ) ) {
						$output = '<nav class="pagination" role="navigation" aria-labelledby="pagination-label">'
						          . '<h2 class="screen-reader-text" id="pagination-label">' . esc_attr__( 'Posts Navigation', 'glamour' ) . '</h2>'
						          . $output
						          . '</nav>';
					}
			...
			function wm_blog_more_link() {
				...
				// Output
					...
					if ( $page_for_posts_id = absint( get_option( 'page_for_posts' ) ) ) {
						...
						echo '<div class="archive-link"><a href="' . esc_url( get_permalink( $page_for_posts_id ) ) . '" class="button">' . esc_html__( 'All posts', 'glamour' ) . '</a></div>';
			...
			if ( ! function_exists( 'wm_food_menu_more_link' ) ) {
				function wm_food_menu_more_link() {
					...
					// Helper variables
						...
						$food_menu_page_id = intval( get_transient( 'glamour-page-template-food-menu' ) );
	...
	if ( ! function_exists( 'wm_food_menu_page_template_id' ) ) {
		function wm_food_menu_page_template_id() {
			...
			// Processing
				...
				if ( false === ( $cached_page_id = get_transient( 'glamour-page-template-food-menu' ) ) ) {
					...
					set_transient( 'glamour-page-template-food-menu', $cached_page_id );
		...
		if ( ! function_exists( 'wm_food_menu_page_template_transient_flusher' ) ) {
			function wm_food_menu_page_template_transient_flusher() {
				...
				// Processing
					...
					delete_transient( 'glamour-page-template-food-menu' );
