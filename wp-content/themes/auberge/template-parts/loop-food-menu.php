 ...
 */
	...
	$glamour_food_menu = new WP_Query( (array) apply_filters( 'wmhook_loop_food_menu_query', array(
 ...
 * Loop
 */
if ( $glamour_food_menu->have_posts() ) :
	...
	<section class="food-menu-items page-section">
		...
		<header class="page-header">
			...
			$glamour_title_food_menu   = (string) apply_filters( 'wmhook_loop_food_menu_title_text', esc_html_x( 'Menu', 'Food menu title.', 'glamour' ) );
			$glamour_food_menu_page_id = intval( get_transient( 'glamour-page-template-food-menu' ) );

			if ( 1 <= $glamour_food_menu_page_id ) {
				$glamour_title_food_menu = '<a href="' . esc_url( get_permalink( $glamour_food_menu_page_id ) ) . '">' . $glamour_title_food_menu . '</a>';
			}

			echo (string) apply_filters( 'wmhook_loop_food_menu_title_text_html', $glamour_title_food_menu );
		...
		<div class="items items-list clearfix"<?php echo wm_schema_org( 'ItemList' ); ?>>
			...
			while ( $glamour_food_menu->have_posts() ) : $glamour_food_menu->the_post();
